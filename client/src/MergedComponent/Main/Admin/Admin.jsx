...
import MeetingPop from './MeetingPop'
import Cookies from "js-cookie";
...
class Admin extends Component{
    ...
    state = {
        ...
        inputElements: ["Email", "User Name", "Password"],
        loading:false
    }
    ...
    dateConverter =  (date) => {
        ...
        if(check[0] === "undefined") completeDate = "-"
    ...
    async handelMeetingSet(){
        await SetMeetingHTTP(this.state.SetMeetingDate)
    }
    handelMeetingComplete = async() =>{
        // await SetMeetingHTTP(this.state.SetMeetingDate)
        this.setState({
    ...
    render(){
        return(
            <div className={(style.bg)}>
                <div className={styles.animateTop} style={{transform: this.state.transform,transformOrigin: "left top"}}>
                    
                    {/* <div className={styles.background} > */}
                        <Link to={"/Main/Admin/Search"}>
                            <button className={styles.searchBar} onClick={this.openPage("SearchPage")}>Search Member</button>
                        </Link>
                        <button className={styles.searchBar} style={{left: "46px"}} onClick={this.handelMeetingComplete}>Complete Meeting</button>
                        <input 
                            type="Text" 
                ...
                { this.state.MeetingComplete &&
                    <MeetingPop
                        open={()=>this.setState({MeetingComplete: false})}
